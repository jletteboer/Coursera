...
```{r appendix, ref.label='libraries, parallel, files,', eval = FALSE}
```
### Appendix A - File details and stats
```{r appendix_a, ref.label='summaries', eval = FALSE}
```
### Appendix B - Sample the data
```{r appendix_b, ref.label='sample', eval = FALSE}
```
### Appendix C - Preprocessing the data
```{r appendix_c, ref.label='preprocess', eval = FALSE}
```
### Appendix D - N-gram Tokenization
```{r appendix_d, ref.label='ngrams', eval = FALSE}
```
### Appendix E - Exploratory Analysis
```{r appendix_e, ref.label='exploratory, unigrams, bigrams, trigrams', eval = FALSE}
```